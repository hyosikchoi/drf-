...
def normalize_view(
    ...
    def request_wrapper(func):
        ...
        def _get_paginated_response(view: GenericViewSet, result: QuerySet | None) -> Response:
            paginated_queryset = view.paginate_queryset(queryset=result)
            if paginated_queryset is not None:
                return view.get_paginated_response(res_serializer_class(paginated_queryset, many=many).data)
            return _get_response(result)

        def _get_response(result: QuerySet | None) -> Response:
